...
uses
  SysUtils, //Classes,
  MFPC.fpJSON, MFPC.JSONScanner, MFPC.JSONParser,
...
(* ********************************************************************
  ...
  TJSONConfig = class(TObject) // (TComponent)
  private
    FFilename: String;
  ...
  protected
    ...
    Procedure LoadFromStream(S : TM_Stream); virtual;
  ...
  public
    ...
    procedure CloseKey;
    procedure ResetKey;
    Procedure EnumSubKeys(Const APath : UnicodeString; List : TJPStrList);
    Procedure EnumValues(Const APath : UnicodeString; List : TJPStrList);
    ...
    Function GetValue(const APath: UnicodeString; AValue: TJPStrList; Const ADefault: String) : Boolean; overload;
    Function GetValue(const APath: UnicodeString; AValue: TJPStrList; Const ADefault: TJPStrList): Boolean; overload;
    ...
    procedure SetValue(const APath: UnicodeString; AValue: TJPStrList; AsObject : Boolean = False); overload;
...
begin
  if Modified then
    begin
    F:=TM_FileStream.Create(FileName,fmCreate);
    Try
      if Formatted then
...
function TJSONConfig.GetValue(const APath: UnicodeString; AValue: TJPStrList;
...
function TJSONConfig.GetValue(const APath: UnicodeString; AValue: TJPStrList;
  const ADefault: TJPStrList): Boolean;
begin
...
procedure TJSONConfig.SetValue(const APath: UnicodeString; AValue: TJPStrList; AsObject: Boolean = False);
var
  El: TJSONData;
  ElName: UnicodeString;
  O: TJSONObject;
...
begin
  ...
  if Assigned(El) then
  begin
    if AsObject then DoDelete := (Not(El is TJSONObject))
    else DoDelete := (Not(El is TJSONArray));
    if DoDelete then
    begin
      I := O.IndexOfName(UTF8Encode(ElName));
      O.Delete(I);
  ...
  If Not Assigned(El) then
  begin
    if AsObject then El := TJSONObject.Create
    else El := TJSONArray.Create;
    O.Add(UTF8Encode(ElName), El);
  end;
  ...
  begin
    A := El as TJSONArray;
    A.Clear;
    //For N in AValue do A.Add(N);
    for i := 0 to AValue.Count - 1 do A.Add(AValue[i]);
  end
  else
  begin
    O := El as TJSONObject;
...
begin
  F:=TM_FileStream.Create(AFileName,fmopenRead or fmShareDenyWrite);
  try
    LoadFromStream(F);
  finally
    F.Free;
...
procedure TJSONConfig.LoadFromStream(S: TM_Stream);
...
procedure TJSONConfig.EnumSubKeys(const APath: UnicodeString; List: TJPStrList);
...
procedure TJSONConfig.EnumValues(const APath: UnicodeString; List: TJPStrList);
